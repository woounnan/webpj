...
export default {
  ...
  methods: {
    setRoom(to){
      ...
      axios.post('webhacker.xyz:8000/apis/db/getChat', {src: this.$store.getters.getUser.id, 'dst': to})
      .then(r =>{
    ...
    recvMsg(){
      this.$store.state.socks.sock.on('msg', data =>{
        ...
        if((data.header.to === $store.getters.getUser.id) && ($store.getters.getRoom.indexOf(data.header.src) != -1)){
            console.log('receive a msg from server')
            this.onNewOwnMessage(data)
        }
      })
    },
    setInput(message, position, image, imageUrl, works){
      const msg = {
        id: this.authorId,
      ...
      this.$store.state.socks.sock.emit('msg', msg)
