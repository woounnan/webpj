...
export default {
  ...
  created() {
        this.$store.state.bus.$on('openDial', (idx)=>{
      
      var to = this.$store.getters.getUser.room[idx]
      console.log('openDial in BasicChat.vue - totototo::::', to)
      console.log('openDial in BasicChat.vue::::', idx)
      this.setRoom(to)
      this.title = to
    })
  ...
  methods: {
    setRoom(to){
      ...
      axios.post('webhacker.xyz:8000/apis/db/getChat', {from: this.$store.getters.getUser.id, to: to})
      .then(r =>{
    ...
    recvMsg(){
      this.$store.state.socks.sock.on('msg', data =>{
        ...
        if((data.header.to === $store.getters.getUser.position) && ($store.getters.getRoom.indexOf(data.header.from) != -1)){
            console.log('receive a msg from server')
            this.onNewOwnMessage(data)
        }
      })
    },
