    ...
    recvMsg(){
      ...
      this.$store.state.bus.$on('work', (data) => {
        ...
        data.selects.forEach(ele=>{
          //보내는 대상이 부서면
          console.log('this.$store.getters.getDivisions::::', this.$store.getters.getDivisions)
          if(this.$store.getters.getDivisions.indexOf(ele) != -1){
            //전체 유저 검색
              for(var i = 0; i<others.length; i++){
                if(others[i].division == ele){

                console.log('selects::::', data.selects)
