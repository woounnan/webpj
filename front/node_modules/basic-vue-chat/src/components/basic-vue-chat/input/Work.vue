...
export default {
  ...
  methods : {
    ...
    sub(){
      ...
      fd.append('by', this.$store.getters.getUser.id)
      fd.append('by_position', this.$store.getters.getUser.position)

      //endDate가 현재날짜보다 뒤에 잇다
      if(this.v_work.notice){
        fd.append('state_c', '미확인')
        fd.append('state_s', '미확인')
      }
      else{
        fd.append('state_c', '미제출')
        fd.append('state_s', '미제출')
      }
      */
      if(this.upFiles.length > 0 )
        fd.append('flag_s_upload', true)
      else
        fd.append('flag_s_upload', false)
      console.log('axios :::')
      //파일 처리 부분(업로드, )
      ...
      .then(r => {
        ...
        var works = {
          ...
          state_c : this.v_work.notice ? '미확인' : '미제출',
          state_s : this.v_work.notice ? '미확인' : '미제출',
          flag_c_upload : false,
          file_s_real : r.data.file_s_real,
          file_s_save : r.data.file_s_save,
        }        
        console.log('selects ::: Work.vue :::', this.v_work.selects)  
        console.log('selects ::: Work.vue :::', works.selects)
    ...
    searchPs(){
      ...
      this.v_searchPs.state = false
      //ps, division 합침
      var arr_m = this.v_ps.concat(this.v_divisions)
      var len = Object.keys(arr_m).length
      for(var i=0; i<len; i++){
        var temp = undefined
        if(arr_m[i].position != undefined){
          //ps면 
          temp = arr_m[i].position
        }else{
          //division이면
          temp = arr_m[i]
        }
