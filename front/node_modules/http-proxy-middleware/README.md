...
// http://192.168.132.129:3000/api/foo/bar -> http://www.example.org/api/foo/bar
```
...
var options = {
  ...
  router: {
    // when request.headers.host == 'dev.192.168.132.129:3000',
    // override target 'http://www.example.org' to 'http://192.168.132.129:8000'
    'dev.192.168.132.129:3000': 'http://192.168.132.129:8000'
  }
}
...
- **option.router**: object/function, re-target `option.target` for specific requests.
  ...
  router: {
      'integration.192.168.132.129:3000' : 'http://192.168.132.129:8001',  // host only
      'staging.192.168.132.129:3000'     : 'http://192.168.132.129:8002',  // host only
      '192.168.132.129:3000/api'         : 'http://192.168.132.129:8003',  // host + path
      '/rest'                      : 'http://192.168.132.129:8004'   // path only
  }

  // Custom router function
  router: function(req) {
      return 'http://192.168.132.129:8004';
  }
  ```
...
- **option.buffer**: stream of data to send as the request body. Maybe you have some middleware that consumes the request stream before proxying it on e.g. If you read the body of a request into a field called 'req.rawbody' you could restream this field in the buffer option:
  ...
  module.exports = (req, res, next) => {
    ...
    proxy.web(req, res, {
      target: 'http://192.168.132.129:4003/',
