...
co(function*() {
  ...
  yield topology.addShard([{
    options: {
      bind_ip: '192.168.132.129', port: 31000, dbpath: `/data/db/31000`, shardsvr: null
    }
  }], { replSet: 'rs1' });

  yield topology.addConfigurationServers([{
    options: {
      bind_ip: '192.168.132.129', port: 35000, dbpath: `/data/db/35000`
    }
  }], { replSet: 'rs0' });

  yield topology.addProxies([{
    bind_ip: '192.168.132.129', port: 51000, configdb: '192.168.132.129:35000'
  }], {
    binary: 'mongos'
  });
